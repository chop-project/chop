...
def analyze_one_file(arg):
    ...
    if analysis_obj != None:
       packages[package].append((ntpath.basename(filename), populate_file(analysis_obj)))
    else:
       packages[package].append((ntpath.basename(filename),  None))
...
def main():
    ...
    for package in elffiles:
        ...
        print_per_file_gadget_summary(package[0], packages[package[0]], "catch")
        print_per_file_gadget_summary(package[0], packages[package[0]], "clean")
